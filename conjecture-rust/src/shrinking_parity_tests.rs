...
fn test_boolean_shrinking_parity() {
    ...
    if let ChoiceValue::Boolean(final_value) = &shrunk_result.choices[0].value {
...
fn test_forced_choice_no_shrinking_parity() {
    ...
    if let ChoiceValue::Integer(final_value) = &shrunk_result.choices[0].value {
        ...
        assert!(shrunk_result.choices[0].was_forced, "Should remain forced");
    } else {
        panic!("Expected integer choice");
    }
}
...
fn test_multi_choice_shrinking_parity() {
    ...
    let shrunk_result = shrinker.shrink(|result| {
        ...
        result.choices.len() >= 3
    });
    
    // Should have same number of choices but minimized values
    assert_eq!(shrunk_result.choices.len(), 3, "Should preserve all choices");
    
    // Check each choice was minimized appropriately
    if let ChoiceValue::Integer(val1) = &shrunk_result.choices[0].value {
        println!("SHRINKING_PARITY: First integer 30 -> {}", val1);
        assert_eq!(*val1, 0, "Should shrink to 0");
    }
    
    if let ChoiceValue::Boolean(val2) = &shrunk_result.choices[1].value {
        println!("SHRINKING_PARITY: Boolean true -> {}", val2);
        assert_eq!(*val2, false, "Should shrink to false");
    }
    
    if let ChoiceValue::Integer(val3) = &shrunk_result.choices[2].value {
